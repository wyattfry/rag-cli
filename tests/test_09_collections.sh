...
if rag_cli chat --allow-commands --auto-approve --prompt 'run echo "collection test"' > /dev/null 2>&1; then
...
if rag_cli chat --allow-commands --auto-approve --auto-index --prompt 'create a file called auto_test_collections.txt with content "auto-indexed content"' > /dev/null 2>&1; then
...
if rag_cli chat --allow-commands --auto-approve --no-history --prompt 'run echo "no history test"' > /dev/null 2>&1; then
