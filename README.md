...
- **Local Models**: Uses locally running language models (e.g., Ollama with larger models for optimal performance)
...
- **Platform-Aware**: Automatically handles macOS vs Linux command syntax differences with flexible command selection
...
# Pull required models (see model recommendations below)
...
4. **Pull Required Models**
   ```bash
   # Pull language model (8B+ recommended for optimal performance)
   ollama pull llama3.1:8b
...
**⚠️ Important: For optimal performance and reliability, use models with 8B parameters or larger. Smaller models (<7B) may significantly hamper functionality and produce poor command execution results.**

#### Language Models (8B+ Recommended)
- **Meta Llama 3.1/3.2 8B+**: Excellent reasoning abilities for complex multi-step tasks
- **Google Gemma 2 9B SimPO**: Superior instruction following for command generation
- **Qwen/CodeQwen 2.5 14B**: Specifically designed for coding and command-line tasks
- **Microsoft Phi 3.5**: Compact but capable option (minimum recommended size)

#### Not Recommended (Too Small)
- **granite-code:3b**: While fast, lacks the reasoning capability for reliable command execution
- **Other models <7B**: Generally insufficient for complex command generation and error recovery
...
```yaml
llm:
  model: "llama3.1:8b"  # or "gemma2:9b", "qwen2.5-coder:14b", etc.
...
```bash
./rag-cli --prompt "create a file called notes.txt with today's date"
```
...
#### System Information
```bash
./rag-cli --prompt "what's the current system load and memory usage?"
```
...
```bash
./rag-cli --prompt "find all Python files and count lines of code"
```
...
```bash
./rag-cli --prompt "create a project structure for a Go web API"
```
...
- **Platform Detection**: Automatic macOS/Linux command syntax handling with flexible command selection
...
- **Flexible Command Selection**: AI can choose from multiple valid approaches for the same task
...
The project includes a comprehensive test suite that validates both RAG functionality and command execution capabilities. Recent improvements ensure test reliability across different platforms and command variations.
...
# Run specific test
./tests/test_02_system_detection.sh
```

Tests now handle multiple valid command approaches for the same task, improving reliability and reducing false failures due to platform-specific command variations.
...
1. **Poor command generation**: Ensure you're using a model with 8B+ parameters
   ```bash
   # Check current model
   ollama list
...
2. **Inconsistent results**: Smaller models may produce unreliable outputs
   - Solution: Upgrade to a recommended 8B+ model
...
4. **Test failures**: Recent updates improve test reliability across platforms
...
./rag-cli chat --auto-approve --prompt "echo 'test'"
```

### Command Execution Issues

#### Commands Not Executing
1. **Missing flag**: Command execution is disabled by default
   ```bash
   ./rag-cli chat  # Enable command execution
   ```

2. **Platform syntax errors**: AI might use different but valid command approaches
   - The AI learns from these interactions and improves over time
   - Historical execution data helps select optimal commands
   - Recent improvements handle multiple valid command types
...
- **Alternative command approaches** for the same task
...
5. **Flexible Command Selection**: AI can choose from multiple valid approaches
...
- Observing alternative command approaches for the same task
...
#### Single Prompt Mode
```bash
./rag-cli chat --prompt "your task here"
```
